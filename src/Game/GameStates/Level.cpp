...
/// @brief фукнция для создания объектов игры 
/// @param description символ, который определяет тип создаваемого объекта
/// @param position позиция, где будет размещен объект
/// @param size размер объекта 
/// @param rotation угол поворота объекта 
...
/// @brief функция для инициализации объекта уровня 
/// @param levelDescription вектор строк в которых описан каждый объект уровня
Level::Level(const std::vector<std::string>& levelDescription, const Game::EGameMode eGameMode)
    : m_eGameMode(eGameMode)//Создаём текущий режим игры
{
...
/// @brief функция для запуска игрового движка
void Level::initLevel()
{
    switch (m_eGameMode)
    {
    case Game::EGameMode::TwoPlayers:
        m_pTank2 = std::make_shared<Tank>(Tank::ETankType::Player2Green_type1, false, true, Tank::EOrientation::Top, 0.05, getPlayerRespawn_2(), glm::vec2(Level::BLOCK_SIZE, Level::BLOCK_SIZE), 0.f);//Конструктор
        Physics::PhysicsEngine::addDynamicGameObject(m_pTank2);
        [[fallthrough]];//fallthrough - показывает что здесь преднамеренно не укзан break  
    case Game::EGameMode::OnePlayer:
        m_pTank1 = std::make_shared<Tank>(Tank::ETankType::Player1Yellow_type1, false, true, Tank::EOrientation::Top, 0.05, getPlayerRespawn_1(), glm::vec2(Level::BLOCK_SIZE, Level::BLOCK_SIZE), 0.f);//Конструктор
        Physics::PhysicsEngine::addDynamicGameObject(m_pTank1);
    }

    //Добавим трёх противников
...
/// @brief функция для отрисовки всех объектов на уровне
void Level::render() const
{
    ...
    for (const auto& currentTank : m_enemyTanks)
    {
        currentTank->render();//Вызываем для каждого конкретнного противника функцию render
    }
}

/// @brief функция для обновления всех объектов на уровне(вызывается при каждом кадре игре)
/// @param delta время, которое прошлого с последнего обновления кадра
...
/// @brief функция для движения танка 
...
/// @brief функция для нахождения всех объектов которые находятся в прямоугольнике 
/// @param bottomLeft нижний угол прямоугольника 
/// @param topRight правый верхний угол прямоугольника
/// @return вектор с указателями на объекты игры, которые находятся в заданном прямоугольнике на экране
