...
bool Game::init()
{
    ...
    std::vector<std::string> subTexturesNames = {//каждая сабтекстура по фотографии
     "block",
     "topBlock", 
     "bottomBlock", 
     "leftBlock", 
    ...
    std::vector<std::string> tanksSubTexturesNames = {//каждая сабтекстура берется поочередно в строчек по 16 пикселей
        "tankTop1",
        "tankTop2", 
        "tankLeft1", 
        "tankLeft2", 
    ...
    pTanksAnimatedSprite->setState("tankTopState");//изначальное положение 
