...
std::shared_ptr<RenderEngine::Texture2D> ResourceManager::loadTexture(const std::string& textureName, const std::string& texturePath)
{
    ...
    if (!pixels){
        std::cerr<<"Can't load inage: " << texturePath << std :: endl;
        return nullptr;
    }
...
bool ResourceManager::loadJSONResources(const std::string& JSONPath)
{
    ...
    auto shadersIt =  document.FindMember("shaders");//Функция FindMember на выходе возвращает итератор
    if (shadersIt != document.MemberEnd())//Проверка на валидность
    {
    ...
    if (textureAtlasesIt != document.MemberEnd())
    {
        for (const auto& currentTextureAtlas : textureAtlasesIt->value.GetArray())
        {
            ...
            loatTextureAtlas(name, filePath, std::move(subTextures), subTextureWidth, subTextureHeight);
        }
    }

    auto spritessIt =  document.FindMember("sprites");
    if (spritessIt != document.MemberEnd())
    {
        for (const auto& currentSprite : spritessIt->value.GetArray())
        {
            ...
            if (!pSprite)
            {
                continue;//переход к следующиму спрайту
            }
    ...
    if (levelsIt != document.MemberEnd())
    {
        for (const auto& currentLevel : levelsIt->value.GetArray())
        {
            ...
            for (auto& currentRow : levelRows)
            {
                ...
                {
                    currentRow.append("D");//добавляет пустой объект
                }
            }
